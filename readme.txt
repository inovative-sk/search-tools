=== Search Tools ===
Contributors: stehlik
Tags: search, search insights, search statistics, search analytics, highlight
Requires at least: 6.2
Tested up to: 6.5
Requires PHP: 7.2
Stable tag: 1.1.1
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-3.0.en.html

Provides tools to boost the search experience. Extends search with post types, meta data, taxonomies, authors and collects statistics.

== Description ==

WP Search Tools is a collection of useful improvements to the site search functionality.

= Search Analytics =

Do you know what people search for on your website, actually? WordPress core doesn’t store any statistical data, but the WP Search Tools plugin shows detailed insights about the history of searched terms. The Insights is a great analytics tool which tells you what users really look for. No personal data is saved. 

= Extend Search =

Free version of the plugin extends the default search engine. It allows searching in post types, taxonomies, authors and meta data. The results are ordered by relevance not post date. All these parts of the WordPress are not included in the basic search. You can customize the settings by yourself to fit your needs.

= Highlight Search Term =

In the search results page, the search term is highlighted with a smooth CSS animation. The highlight MUST BE ENABLED first! User experience is enhanced and visual mark speeds up finding relevant post. Furthermore, highlight settings are customizable, both background colour and text colour can be set in the WordPress administration options page.

= Key Features =

* collects search terms what people look for
* displays graph with recent data
* highlights search term
* sorts data according to the users (logged in, guests) and dates
* highlights the term if it has no results
* shows basic overview on the Dashboard
* extends default search engine with post types, meta data, taxonomies, meta data
* orders search results by relevance
* settings are customizable

== Links ==

* [Website](https://www.wpsearchtools.com/)
* [Documentation](https://docs.wpsearchtools.com/)
* [GitHub repository](https://github.com/inovative-sk/search-tools)

== Screenshots ==

1. Highlight Search Term
2. Search Insights
3. The Graph of the Searches
4. Settings 
5. How to Zoom the Graph
6. Admin Menu Item